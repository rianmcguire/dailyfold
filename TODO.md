...
- **Group operations on a block selection** — block selection ships (Alt+↑/↓ enters, ↑/↓ moves, Shift+↑/↓ extends, ←/→ exits to edit, Esc clears). Still TODO: Tab/Shift+Tab to indent/outdent the group; Backspace/Delete to delete it; typing replaces it; shift+click to extend selection from the current edit point.
- **Move blocks** — drag or keyboard (Alt+Shift+↑/↓, since Alt+↑/↓ now enters selection mode) to reorder within siblings; also "move into" / "move out of" parent.
